...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/dbcolberHW5"
...
class User(db.Model):
    ...
    email = db.Column(db.String(64)) # Adding email to class User so it can be in the db table!
    def __repr__(self):
...
def index():
    ...
    if form.validate_on_submit():
        if db.session.query(Tweet).filter_by(text=form.text.data, user_id= (get_or_create_user(db.session, form.username.data, form.email.data).id)).first():
            ...
            if app.config['ADMIN']:
                send_email(app.config["ADMIN"], 'You have a new Tweet!', 'mail/new_tweet', tweet=form.text.data)
